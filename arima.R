...
# Take a regression vector
dt[, Price:=1/Price]
m_reg <- dt[, .(Price)]
m_reg <- apply(X = m_reg, MARGIN = 2, FUN = transform$standardize)
m_reg <- ts(m_reg, frequency = 52)
...
m_reg_train <- subset(x = m_reg, start = 1, end = t - test_size)
m_reg_test <- subset(x = m_reg, start = t - test_size + 1, end = t)
...
Arima_forecast_CV <- function(x, h, xreg, order, sorder){
  forecast(Arima(x, order = order, seasonal = sorder, xreg = xreg[seq_along(x)]), 
           h = h, xreg = xreg[seq(h) + length(x)])
}

# Take the order parameters on the full set and get the order of the model
system.time(
ss_arima_fit <- auto.arima(y = v, seasonal = T, xreg = m_reg,
...
CV <- models$tsCV(y = v,forecastfunction = Arima_forecast_CV, h = 52, 
                  initial = 52, xreg = m_reg,
                  step = 10, parallel = T, clusterSize = 8,
                  order = order, sorder = sorder)
CV_error <- cross_validation$get_tsCV_error(CV = CV)*sd

#Series: train_v 
#Regression with ARIMA(0,1,1)(1,1,1)[52] errors 
#
#Coefficients:
#  ma1     sar1     sma1    xreg
#-0.8140  -0.1183  -0.5359  0.2103
#s.e.   0.0477   0.1799   0.2233  0.0999

#sigma^2 estimated as 0.03888:  log likelihood=31.55
#AIC=-53.1   AICc=-52.8   BIC=-36.44

# training and testing errors
# train the model on the train set and take the regression coefficients
ss_arima_model<- try(Arima(y = train_v, xreg = m_reg_train, order = order, 
                           seasonal = sorder))
if(class(ss_arima_model)=="try-error"){
  ss_arima_model<- Arima(y = train_v, xreg = m_reg_train, order = order, 
...
arima_predict <- forecast(ss_arima_model, h = 52, xreg = m_reg_test)
...
arima_fit_final <- Arima(y = v, xreg = m_reg, model = ss_arima_model)

forecast <- forecast(arima_fit_final, h = 52, 
                     xreg = matrix(rep(last(m_reg), 52), nrow = 52, ncol = 1, 
                                   dimnames = list(seq_along(1:52),"Price")))
...
# Forecast function using ARIMA
Arima_forecast_CV_order <- function(x, h, xreg, order, s_order){
  forecast(Arima(x, order = order, seasonal = s_order,xreg = xreg[seq_along(x)], method = 'ML'), 
           h = h, xreg = xreg[seq(h) + length(x)])
}
...
system.time(
  res <- models$Get_params_CV(ts = v, d = d_values$d, ds = d_values$ds, seasonal = T, 
                            xreg = m_reg, clusterSize = 8, forecastfunction = Arima_forecast_CV_order,
...
CV_arima_fit <- Arima(y = train_v, order = order, seasonal = s_order, 
                      xreg = as.matrix(m_reg_train))
CV_arima_residuals <- residuals(CV_arima_fit)
CV_arima_fitting_error <- sqrt(mean(CV_arima_residuals^2))*sd

# prediction & prediction error
CV_arima_predict <- forecast(CV_arima_fit, h = 52, xreg = m_reg_test)
prediction_error <- rmse(test_v, CV_arima_predict$mean)*sd

# Train the model parameters using the full dataset, but keeping the model order
final_fit <- try(Arima(y = v, xreg = m_reg, model = CV_arima_fit), silent = T)

# Get the actual forecast
forecast <- forecast(final_fit, h = horizon, xreg = matrix(rep(tail.matrix(m_reg, n = 1), 
                                                               horizon), nrow = horizon, ncol = 1, byrow = T, 
                                                           dimnames = list(seq(1, horizon), "Price")))
...
fwrite(new_dt, "data/arima.csv", append = F)
#new_dt <- fread("data/arima.csv")
